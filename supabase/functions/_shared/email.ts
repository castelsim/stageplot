...
export function buildEmailHtml(
  ...
  const rows: [string, string | undefined][] = [
    ["Nome", b.name],
    ["Email", b.email],
    ["Tipo evento", b.event_type],
    ["Data e luogo", b.date_place],
    ["Organico", b.lineup],
    ["Materiali", b.materials],
    ["Note", b.notes],
  ];
  const body = rows.map(([k, v]) => `<p><strong>${k}:</strong> ${esc(v)}</p>`)
    .join("");
  const links = opts.attachmentUrls.length
    ? "<p><strong>Allegati:</strong></p><ul>" +
      opts.attachmentUrls.map((u) => `<li><a href="${u}">${u}</a></li>`).join(
        "",
      ) + "</ul>"
  ...
  const view = opts.viewUrl
    ...
    : "";
  const html =
    `<h2>Richiesta consulenza Stage Plot</h2><p><strong>Stato pagamento:</strong> ${stato}</p>${body}${view}${links}`;
  return { subject, html };
}
